...
class WeatherService {
  ...
  static Future<void> getCurrentWeather({required String? cityName, required BuildContext context}) async {
    WeatherModel weatherModel;
    final response = await http
        .get(Uri.parse('$kBASEURL/weather?q=$cityName&appid=$apiKey&units=metric'));
    if (response.statusCode == 200) {
      weatherModel = WeatherModel.fromJson(
          jsonDecode(response.body));

      if(context.mounted){
        context.read<CurrentWeatherProvider>().updateCurrentWeather(weatherModel);
      }
    } else {
  ...
  static Future<String> getCurrentCity(BuildContext context) async {
    ...
    double latitude = Provider.of<CoordinateProvider>(context, listen: false).latitude;
  ...
  static Future<void> getCoordinates(BuildContext context)async{
    ...
    if(context.mounted){
      context.read<CoordinateProvider>().updateCoordinates(lat: position.latitude, lon: position.longitude);
    }
  ...
  static Future<void> getWeatherForecast(BuildContext context)async{
    ...
    final response = await http
        .get(Uri.parse('$kBASEURL/forecast?lat=$latitude&lon=$longitude&appid=$apiKey&units=metric'));
    if (response.statusCode == 200) {
      for(int i = 0; i < 3; i++){
        forecastWeatherModelList.add(ForecastWeatherModel.fromJson(jsonDecode(response.body), i));
      }

      //FORECAST WEATHER PROVIDER UPDATED
      if(context.mounted){
        context.read<ForecastWeatherProvider>().updateForecastWeather(forecastWeatherModelList);
      }
    } else {
      throw Exception('Failed to load weather data');
    }
